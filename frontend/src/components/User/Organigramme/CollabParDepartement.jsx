...
const CollabParDepartement = (departToView) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/api/departement/${departToView.departToView.id}/collaborateur`)
      .then(response => {
      ...
      {collab.map((collaborateur) => (
        <div className="col-span-4"  key={collaborateur.id}>
          <div className="flex flex-row">
            <Avatar src={`http://localhost:8000/${collaborateur.image}`} alt={collaborateur.nom} className="m-5" size="xl"/>
